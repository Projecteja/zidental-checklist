...
function loadTemplatesForBranch(branch) {
  return loadTemplates().filter((t) => t.branch === branch);
}
...
function seedIfEmpty() {
  ...
  const templatesLookValid = existingTemplates.length > 0 && existingTemplates.every((t) => 'branch' in t);
  ...
  branchesToSeed.forEach((branch) => {
    DEFAULT_TEMPLATES.forEach((t) => {
      allTemplates.push({ id: uid('tpl'), branch, ...t });
    });
  });
...
function addTaskTemplate(data) {
  ...
  const tpl = { id: uid('tpl'), branch: getCurrentBranch(), ...data };
  templates.push(tpl);
  saveTemplates(templates);

  // Also add to today's checklist immediately.
  const record = ensureTodayRecord();
  record.tasks.push(buildTaskFromTemplate(tpl, record.date, tpl.branch));
  saveRecord(record);
