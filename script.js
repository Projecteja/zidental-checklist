...
const ROLES = ['Super Admin', 'Head of Clinic', 'Reception', 'Dental Assistant', 'Admin'];
...
const ADMIN_ROLES = ['Head of Clinic', 'Super Admin'];
// Roles that are not tied to a single branch — they can see/manage every branch.
const ALL_BRANCH_ROLES = ['Super Admin'];
...
// Each entry: { id, name, role, branch, username, password }
// branch is null for roles in ALL_BRANCH_ROLES (e.g. Super Admin), who see every branch.
...
const DEFAULT_STAFF = [
  { id: 'usr_owner', name: 'Pak Zaki', role: 'Super Admin', branch: null, username: 'owner', password: 'owner123' },
  { id: 'usr_dhimas', name: 'Dhimas', role: 'Head of Clinic', branch: 'Karawaci', username: 'dhimas', password: 'admin123' },
  { id: 'usr_meli', name: 'Meli', role: 'Reception', branch: 'Karawaci', username: 'meli', password: 'meli123' },
  { id: 'usr_widia', name: 'Widia', role: 'Dental Assistant', branch: 'Karawaci', username: 'widia', password: 'widia123' },
  { id: 'usr_rani', name: 'Rani', role: 'Admin', branch: 'Karawaci', username: 'rani', password: 'rani123' },
  { id: 'usr_fajar', name: 'Fajar', role: 'Head of Clinic', branch: 'Ciledug', username: 'fajar', password: 'fajar123' },
  { id: 'usr_sinta', name: 'Sinta', role: 'Reception', branch: 'Ciledug', username: 'sinta', password: 'sinta123' },
];

// Base set of tasks used to seed EACH branch with its own copy of templates
// (each branch's Head of Clinic manages their own templates independently).
...
const state = {
  ...
  viewingBranch: null, // only used by Super Admin to switch which branch they're viewing
  selectedTemplateIds: new Set(), // Tasks page bulk-select
};

/* ------------------------------------------------------------
   2. STORAGE HELPERS
   ------------------------------------------------------------ */
// Reverse-lookup so saveJSON (which is called with the raw localStorage
// key string, e.g. 'task_templates') knows which short CloudSync key
// (e.g. 'templates') to push under. clinic_session is intentionally
// excluded — login sessions are per-device, not synced.
const CLOUD_SYNC_KEYS = Object.fromEntries(
  Object.entries(LS_KEYS)
    .filter(([short]) => short !== 'session')
    .map(([short, raw]) => [raw, short])
);
...
function saveJSON(key, value) {
  ...
  const cloudKey = CLOUD_SYNC_KEYS[key];
  if (cloudKey && window.CloudSync && window.CloudSync.isAvailable) {
    window.CloudSync.push(cloudKey, value);
  }
}
...
// Templates now store `branches: [...]` (an array) instead of a single
// `branch` string, so one template can apply to several branches, or to
// every branch via the special value 'All'. loadTemplatesForBranch() reads
// old-format templates (single `branch` string) transparently too, so
// existing data keeps working without a migration step.
function templateAppliesToBranch(tpl, branch) {
  if (Array.isArray(tpl.branches)) {
    return tpl.branches.includes('All') || tpl.branches.includes(branch);
  }
  return tpl.branch === branch; // legacy single-branch templates
}
function loadTemplatesForBranch(branch) {
  return loadTemplates().filter((t) => templateAppliesToBranch(t, branch));
}
...
function recordKey(dateKey, branch) {
  return `${dateKey}__${branch}`;
}
...
function seesAllBranches(user) {
  return !!user && ALL_BRANCH_ROLES.includes(user.role);
}

/* The branch currently being viewed/worked in. For a branch-locked staff
   member this is always their own branch. For an all-branch role (Super
   Admin) it's whichever branch they've picked via the branch switcher,
   defaulting to the first branch that has any staff. */
function getCurrentBranch() {
  const user = getActiveUser();
  if (!user) return null;
  if (!seesAllBranches(user)) return user.branch;
  if (state.viewingBranch) return state.viewingBranch;
  const staff = loadStaffDirectory();
  const firstBranch = (staff.find((s) => s.branch) || {}).branch || BRANCHES[0];
  state.viewingBranch = firstBranch;
  return firstBranch;
}

function setViewingBranch(branch) {
  state.viewingBranch = branch;
}
...
function attemptLogin(username, password) {
  ...
  if (!user) return null;
  saveSession(user.id);
  state.viewingBranch = seesAllBranches(user) ? null : user.branch;
  return user;
}

function logout() {
  clearSession();
  state.viewingBranch = null;
  renderLoginScreen();
...
function seedIfEmpty() {
  ...
  // from an older version of this app (no username/password/branch fields)
  // doesn't leave a broken/incompatible account list behind.
  const existingStaff = loadJSON(LS_KEYS.staff, null);
  const staffLooksValid = Array.isArray(existingStaff) && existingStaff.length > 0 &&
    existingStaff.every((s) => s && s.username && s.password && ('branch' in s));
  if (!staffLooksValid) {
    saveStaffDirectory(DEFAULT_STAFF);
  }

  const existingTemplates = loadTemplates();
  const templatesLookValid = existingTemplates.length > 0 && existingTemplates.every((t) => 'branch' in t || 'branches' in t);
  if (templatesLookValid) return;

  // Migrating from a pre-branch version (or fresh install): wipe any
  // stale, branch-less daily records/logs so old data doesn't linger
  // alongside the new branch-scoped structure.
  saveRecords({});
  saveLogs([]);
  ...
  // Seed every branch that has staff assigned to it with its own copy of
  // the default task templates, so each branch's checklist is independent.
  const staffForSeed = loadStaffDirectory();
  const branchesToSeed = Array.from(new Set(staffForSeed.map((s) => s.branch).filter(Boolean)));
  if (branchesToSeed.length === 0) branchesToSeed.push(BRANCHES[0]);

  const allTemplates = [];
  branchesToSeed.forEach((branch) => {
    DEFAULT_TEMPLATES.forEach((t) => {
      allTemplates.push({ id: uid('tpl'), branches: [branch], ...t });
    });
  });
  ...
  // Seed 2 previous days per branch with realistic completion so
  // History/Reports have data to show.
  branchesToSeed.forEach((branch) => {
    const branchStaff = staffForSeed.filter((s) => s.branch === branch);
    const demoStaffByAssignee = {};
    ['Reception', 'Dental Assistant', 'Head of Clinic', 'Admin'].forEach((role) => {
      const match = branchStaff.find((s) => s.role === role);
      if (match) demoStaffByAssignee[role] = { name: match.name, role: match.role };
    });
    demoStaffByAssignee['All Staff'] = demoStaffByAssignee['Reception'] || { name: 'Staff', role: 'Reception' };

    const templates = allTemplates.filter((t) => t.branch === branch);
    ...
    [-2, -1].forEach((offset, dayIdx) => {
      ...
      const tasks = templates.map((tpl, i) => {
        const t = buildTaskFromTemplate(tpl, dateKey, branch);
        const shouldComplete = (i / templates.length) < completionTarget;
        if (shouldComplete) {
          const completer = demoStaffByAssignee[tpl.assignedTo] || demoStaffByAssignee['Reception'] || { name: 'Staff', role: tpl.assignedTo };
      ...
      records[recordKey(dateKey, branch)] = {
        date: dateKey,
        branch,
        tasks,
...
/* Ensure today's daily record exists for the given branch (auto daily
   snapshot / reset). Defaults to the current user's branch. */
function ensureTodayRecord(branch) {
  const targetBranch = branch || getCurrentBranch();
  if (!targetBranch) return { date: todayKey(), branch: null, tasks: [], totalTasks: 0, completedTasks: 0, completionRate: 0 };
  const records = loadRecords();
  const key = recordKey(todayKey(), targetBranch);
  if (!records[key]) {
    const templates = loadTemplatesForBranch(targetBranch);
    const tasks = templates.map((tpl) => buildTaskFromTemplate(tpl, todayKey(), targetBranch));
    records[key] = {
      date: todayKey(),
      branch: targetBranch,
      tasks,
...
function getRecord(dateKey, branch) {
  const targetBranch = branch || getCurrentBranch();
  const records = loadRecords();
  return records[recordKey(dateKey, targetBranch)] || null;
}

function getAllRecordsForBranch(branch) {
  const targetBranch = branch || getCurrentBranch();
  const records = loadRecords();
  const out = {};
  Object.keys(records).forEach((key) => {
    const rec = records[key];
    if (rec.branch === targetBranch) out[rec.date] = rec;
  });
  return out;
}
...
function saveRecord(record) {
  ...
  records[recordKey(record.date, record.branch)] = record;
  saveRecords(records);
}
...
function computeDisplayStatus(task) {
  ...
  if (task.date < todayKey()) {
    // A past day's task that never got completed is Missed — the day it
    // was due has already gone by, so "Pending" no longer makes sense.
    return 'Missed';
  }
...
// `note` is optional free text — used for reporting an issue/kendala when
// completing a task, or explaining why a task is being reset to pending.
function toggleTaskStatus(taskId, note) {
...
function resetTaskToPending(taskId, note) {
  const record = ensureTodayRecord();
  const task = record.tasks.find((t) => t.id === taskId);
  if (!task) return;
  task.status = 'Pending';
...
// `data.branches` is an array — either ['All'] or a list of specific branch
// names picked in the modal's checkbox group. One template is created but
// it applies to every branch listed; each branch's today-record still gets
// its own independent task instance (so completing it in one branch never
// affects another).
function addTaskTemplate(data) {
  const templates = loadTemplates();
  const { branches, ...rest } = data;
  const tpl = { id: uid('tpl'), branches: branches && branches.length ? branches : [getCurrentBranch()], ...rest };
  templates.push(tpl);
  saveTemplates(templates);

  // Also add to today's checklist immediately, for every branch this
  // template now applies to.
  const targetBranches = tpl.branches.includes('All') ? BRANCHES : tpl.branches;
  targetBranches.forEach((branch) => {
    const record = ensureTodayRecord(branch);
    record.tasks.push(buildTaskFromTemplate(tpl, record.date, branch));
    saveRecord(record);
  });

  logActivity('created', tpl.title);
  const scopeLabel = tpl.branches.includes('All') ? 'all branches' : tpl.branches.join(', ');
  showToast('Task added', `"${tpl.title}" added to ${scopeLabel}`);
  renderAll();
}
...
function editTaskTemplate(templateId, data) {
  ...
  if (!tpl) return;
  const { branches, ...rest } = data;
  Object.assign(tpl, rest);
  if (branches && branches.length) tpl.branches = branches;
  delete tpl.branch; // drop legacy single-branch field once edited
  saveTemplates(templates);

  // Patch today's matching task in every branch this template applies to
  // (title/category/priority/assignedTo/required/dueTime only).
  const targetBranches = tpl.branches.includes('All') ? BRANCHES : tpl.branches;
  targetBranches.forEach((branch) => {
    const record = ensureTodayRecord(branch);
...
function deleteTaskTemplate(templateId) {
  ...
  const tpl = templates.find((t) => t.id === templateId);
  if (!tpl) return;
  const tplBranches = Array.isArray(tpl.branches) ? tpl.branches : [tpl.branch];
  saveTemplates(templates.filter((t) => t.id !== templateId));

  // Remove from today's record too, in every branch it applied to (past
  // history stays untouched).
  const targetBranches = tplBranches.includes('All') ? BRANCHES : tplBranches;
  targetBranches.forEach((branch) => {
    const record = ensureTodayRecord(branch);
...
function filterTasks(tasks, filter) {
  ...
  return tasks.filter((t) => {
    ...
    const statusOk =
      ...
      (filter.status === 'Pending' && t.status !== 'Completed' && displayStatus !== 'Missed') ||
      (filter.status === 'Missed' && displayStatus === 'Missed');
...
function renderHeader() {
  ...
  document.getElementById('clinicBranchHeader').textContent = branch || '—';
  ...
  renderBranchSwitcher(user);
}

/* Hide nav items / controls that this role shouldn't see. Everyone sees
   Dashboard, Daily Checklist, History, Reports. Only Head of Clinic /
   Super Admin see Tasks (task templates management) and Access Control
   (in Settings). */
...
/* Branch switcher: only shown to all-branch roles (Super Admin), letting
   them pick which branch's data they're currently viewing/managing. */
function renderBranchSwitcher(user) {
  ...
  wrap.style.display = '';
  const select = document.getElementById('branchSwitcherSelect');
  const current = getCurrentBranch();
  select.innerHTML = BRANCHES.map((b) => `<option value="${escapeHTML(b)}" ${b === current ? 'selected' : ''}>${escapeHTML(b)}</option>`).join('');
}
...
function renderDashboard() {
  ...
  // Recent activity
  const logs = loadLogs().filter((l) => l.branch === getCurrentBranch()).slice(0, 8);
...
function renderStaffPerformance(container, tasks) {
  const staff = loadStaffDirectory().filter((s) => s.branch === getCurrentBranch());
...
function renderChecklist() {
  ...
  groupsEl.querySelectorAll('[data-check]').forEach((el) => {
    el.addEventListener('click', () => openNoteModal(el.dataset.check));
  });
...
function renderTaskCard(task) {
  ...
  if (displayStatus === 'Completed') {
    ...
    statusHtml = `<span class="task-status-text ${statusClass}">Missed<span class="task-status-time">${formatDateShort(task.date)}</span></span>`;
  ...
  return `
    <div class="task-card ${completed ? 'completed' : ''} ${displayStatus === 'Overdue' ? 'overdue' : ''}">
      ...
      <div class="task-main">
        ...
        ${task.note ? `<div class="task-note"><span class="task-note-icon">📝</span><span>${escapeHTML(task.note)}</span></div>` : ''}
      </div>
      <div class="task-side">
        ${statusHtml}
...
function renderTasksPage() {
  const templates = loadTemplatesForBranch(getCurrentBranch());
  const canManage = canManageTasks();
  ...
  const resetPanel = document.getElementById('periodicResetPanel');
  if (resetPanel) resetPanel.style.display = canManage ? '' : 'none';

  // Drop any selected ids that no longer exist in this branch's template
  // list (e.g. after switching branches or a delete elsewhere).
  const validIds = new Set(templates.map((t) => t.id));
  state.selectedTemplateIds = new Set([...state.selectedTemplateIds].filter((id) => validIds.has(id)));

  const bulkBar = document.getElementById('bulkActionsBar');
  if (bulkBar) bulkBar.style.display = canManage && templates.length > 0 ? '' : 'none';
  ...
  if (templates.length === 0) {
    ...
    renderBulkSelectionUI(templates);
    return;
  }

  container.innerHTML = templates.map((tpl) => {
    const tplBranches = Array.isArray(tpl.branches) ? tpl.branches : [tpl.branch];
    const branchLabel = tplBranches.includes('All') ? 'All Branches' : tplBranches.join(', ');
    const checked = state.selectedTemplateIds.has(tpl.id);
    return `
    <div class="template-row ${checked ? 'selected' : ''}" data-row-tpl="${tpl.id}">
      ${canManage ? `<input type="checkbox" class="template-row-check" data-select-tpl="${tpl.id}" ${checked ? 'checked' : ''} aria-label="Select task" />` : `<span></span>`}
      <div>
        ...
        <div class="task-meta">
          ...
          <span class="badge badge-required">${escapeHTML(branchLabel)}</span>
        </div>
      </div>
  ...
  container.querySelectorAll('[data-select-tpl]').forEach((cb) => {
    cb.addEventListener('click', (e) => e.stopPropagation());
    cb.addEventListener('change', (e) => {
      const id = e.target.dataset.selectTpl;
      if (e.target.checked) state.selectedTemplateIds.add(id);
      else state.selectedTemplateIds.delete(id);
      e.target.closest('.template-row').classList.toggle('selected', e.target.checked);
      renderBulkSelectionUI(templates);
    });
  });
...
/* Keeps the "select all" checkbox and the selected-count label in sync
   with state.selectedTemplateIds, without re-rendering the whole table
   (so an in-progress click doesn't get wiped mid-interaction). */
function renderBulkSelectionUI(templates) {
  ...
  if (!selectAll || !countLabel || !deleteBtn) return;

  const count = state.selectedTemplateIds.size;
  countLabel.textContent = count > 0 ? `${count} selected` : 'Select all';
  deleteBtn.disabled = count === 0;
  deleteBtn.style.opacity = count === 0 ? '0.5' : '1';
  deleteBtn.style.cursor = count === 0 ? 'not-allowed' : 'pointer';

  selectAll.checked = templates.length > 0 && count === templates.length;
  selectAll.indeterminate = count > 0 && count < templates.length;
}

function bulkDeleteSelectedTemplates() {
  const templates = loadTemplatesForBranch(getCurrentBranch());
  const ids = [...state.selectedTemplateIds];
  const toDelete = templates.filter((t) => ids.includes(t.id));
  if (toDelete.length === 0) return;

  openConfirmModal(
    `Hapus ${toDelete.length} task terpilih? Task-task ini akan hilang dari daftar template dan checklist hari ini.`,
    () => {
      toDelete.forEach((tpl) => deleteTaskTemplateSilent(tpl.id));
      state.selectedTemplateIds.clear();
      showToast('Tasks deleted', `${toDelete.length} task dihapus`);
      renderAll();
    },
...
// Same as deleteTaskTemplate but skips its own toast/re-render, so a bulk
// delete of many tasks doesn't spam toasts or re-render on every item.
function deleteTaskTemplateSilent(templateId) {
  const templates = loadTemplates();
  const tpl = templates.find((t) => t.id === templateId);
  if (!tpl) return;
  const tplBranches = Array.isArray(tpl.branches) ? tpl.branches : [tpl.branch];
...
/* ------------------------------------------------------------
   11b. PERIODIC RESET (clear daily records / history for a branch,
   e.g. wiping a month's worth of checklist history without touching
   the task templates themselves)
   ------------------------------------------------------------ */
function resetRecordsForBranch(branch, scope) {
  const records = loadRecords();
  const monthPrefix = todayKey().slice(0, 7); // "YYYY-MM"
  let removed = 0;
  Object.keys(records).forEach((key) => {
    const rec = records[key];
    if (rec.branch !== branch) return;
    if (scope === 'month' && !rec.date.startsWith(monthPrefix)) return;
    delete records[key];
    removed++;
  });
  saveRecords(records);

  // Also clear matching activity logs for this branch/scope so Reports
  // and Recent Activity don't show stale entries pointing at wiped days.
  const logs = loadLogs().filter((l) => {
    if (l.branch !== branch) return true;
    if (scope === 'month') return !l.date.startsWith(monthPrefix);
    return false;
  });
  saveLogs(logs);
...
function periodicResetHistory(scope) {
  const branch = getCurrentBranch();
  const removed = resetRecordsForBranch(branch, scope);
  ensureTodayRecord(branch);
  showToast('History reset', `${removed} record${removed === 1 ? '' : 's'} cleared for ${branch}`);
  renderAll();
}
...
function getAllRecordDates() {
  const records = getAllRecordsForBranch(getCurrentBranch());
  ...
  let html = completedTasks.map((t) => `
    <div class="timeline-item">
      ...
      <div class="timeline-body">
        ...
        ${t.note ? `<div class="task-note"><span class="task-note-icon">📝</span><span>${escapeHTML(t.note)}</span></div>` : ''}
      </div>
    </div>
  `).join('');
...
function renderSettingsPage() {
  ...
  // Clinic Settings is Super Admin only — Head of Clinic and below can see
  // the current name but can't edit or save it.
  const isSuperAdmin = seesAllBranches(user);
  document.getElementById('settingClinicName').disabled = !isSuperAdmin;
  document.getElementById('saveClinicSettingsBtn').style.display = isSuperAdmin ? '' : 'none';
  document.getElementById('clinicSettingsReadonlyNotice').hidden = isSuperAdmin;

  // Profile card is read-only info + password change (identity/role/branch
  // is managed centrally via Access Control, not self-edited).
  ...
  document.getElementById('profileBranchDisplay').textContent = seesAllBranches(user) ? 'All Branches' : (user.branch || '—');
  document.getElementById('profileUsernameDisplay').textContent = user.username;
  document.getElementById('newPasswordInput').value = '';

  // Access Control panel: only render/show for Head of Clinic / Super Admin.
  const accessPanel = document.getElementById('accessControlPanel');
  if (accessPanel) {
  ...
  // Reset All Clinic Data is destructive cloud-wide (wipes every branch
  // for every device once Firebase sync is on) — Super Admin only.
  const resetPanel = document.getElementById('resetDataPanel');
  if (resetPanel) resetPanel.style.display = seesAllBranches(user) ? '' : 'none';
}

function saveClinicSettings() {
  const user = getActiveUser();
  if (!seesAllBranches(user)) {
    showToast('Not allowed', 'Only Super Admin can change clinic settings', 'error');
    return;
  }
  const settings = loadSettings();
...
/* ------------------------------------------------------------
   14b. ACCESS CONTROL (Head of Clinic / Super Admin) — manage staff
   accounts. Head of Clinic only sees & manages accounts in their own
   branch. Super Admin sees & manages accounts in whichever branch
   they're currently viewing (via the branch switcher), plus other
   Super Admin accounts.
   ------------------------------------------------------------ */
function accessControlScope() {
  const me = getActiveUser();
  const staff = loadStaffDirectory();
  if (seesAllBranches(me)) {
    const branch = getCurrentBranch();
    return staff.filter((s) => s.branch === branch || seesAllBranches(s));
  }
  return staff.filter((s) => s.branch === me.branch);
}

function renderAccessControlList() {
  const list = document.getElementById('accessControlList');
  if (!list) return;
  const staff = accessControlScope();
  const me = getActiveUser();

  const branchTag = document.getElementById('accessControlBranchTag');
  if (branchTag) {
    branchTag.textContent = seesAllBranches(me) ? `— viewing ${getCurrentBranch()}` : '';
  }
  ...
  list.innerHTML = staff.map((s) => `
    <div class="access-row" data-user-id="${escapeHTML(s.id)}">
      ...
      <div class="staff-row-info">
        ...
        <div class="staff-row-role">${escapeHTML(s.role)} · ${seesAllBranches(s) ? 'All Branches' : escapeHTML(s.branch || '—')} · @${escapeHTML(s.username)}</div>
      </div>
...
function updateAccessBranchFieldVisibility() {
  const roleSelect = document.getElementById('accessFieldRole');
  const branchField = document.getElementById('accessFieldBranchWrap');
  const isAllBranch = ALL_BRANCH_ROLES.includes(roleSelect.value);
  branchField.style.display = isAllBranch ? 'none' : '';
}
...
function openAccessModal(mode, userId) {
  ...
  const me = getActiveUser();
  const branchSelect = document.getElementById('accessFieldBranch');
  branchSelect.innerHTML = BRANCHES.map((b) => `<option value="${escapeHTML(b)}">${escapeHTML(b)}</option>`).join('');

  const roleSelect = document.getElementById('accessFieldRole');
  // Head of Clinic can only create/manage roles within their own branch's
  // team (not Super Admin). Super Admin can assign any role.
  const availableRoles = seesAllBranches(me) ? ROLES : ROLES.filter((r) => r !== 'Super Admin');
  roleSelect.innerHTML = availableRoles.map((r) => `<option value="${escapeHTML(r)}">${escapeHTML(r)}</option>`).join('');
  ...
  if (mode === 'edit') {
    ...
    branchSelect.value = s.branch || getCurrentBranch();
    ...
    branchSelect.value = seesAllBranches(me) ? getCurrentBranch() : me.branch;
    document.getElementById('accessFieldPassword').placeholder = '';
  }
  updateAccessBranchFieldVisibility();
...
function submitAccessForm(e) {
  ...
  const branch = isAllBranch ? null : document.getElementById('accessFieldBranch').value;
  ...
  if (state_editingUserId) {
    ...
    staff.push({ id: uid('usr'), name, role, branch, username, password });
    saveStaffDirectory(staff);
    showToast('Account added', `${name} · ${role}`);
  }

  // If a Super Admin just added/edited an account in a branch other than
  // the one they're currently viewing, switch the Branch Switcher to that
  // branch too — otherwise the account they just saved would immediately
  // disappear from the list (it's still saved correctly, just filtered
  // out of view), which looks like a bug.
  const me = getActiveUser();
  if (seesAllBranches(me) && branch && branch !== getCurrentBranch()) {
...
/* ------------------------------------------------------------
   ...
   ------------------------------------------------------------ */
// Renders the "Branches" checkbox group in the task modal. Only shown to
// all-branch roles (Super Admin) — a branch-locked user (Head of Clinic)
// can only ever manage tasks for their own branch, so the picker is
// pointless (and potentially confusing) for them and is hidden instead.
function renderTaskModalBranchGroup(selectedBranches) {
  const wrap = document.getElementById('fieldBranchesWrap');
  const group = document.getElementById('fieldBranchesGroup');
  const user = getActiveUser();
  ...
  const isAll = selectedBranches.includes('All');
  group.innerHTML = `
    <label class="all-branches-opt"><input type="checkbox" value="All" id="fieldBranchAll" ${isAll ? 'checked' : ''}/> All Branches</label>
    ${BRANCHES.map((b) => `<label><input type="checkbox" value="${escapeHTML(b)}" class="branch-opt" ${isAll || selectedBranches.includes(b) ? 'checked' : ''} ${isAll ? 'disabled' : ''}/> ${escapeHTML(b)}</label>`).join('')}
  `;

  document.getElementById('fieldBranchAll').addEventListener('change', (e) => {
    const checked = e.target.checked;
    group.querySelectorAll('.branch-opt').forEach((cb) => {
      cb.checked = checked;
      cb.disabled = checked;
    });
  });
...
function getSelectedBranchesFromModal() {
  const user = getActiveUser();
  if (!seesAllBranches(user)) return [getCurrentBranch()];
  const allChecked = document.getElementById('fieldBranchAll')?.checked;
  if (allChecked) return ['All'];
  const picked = Array.from(document.querySelectorAll('#fieldBranchesGroup .branch-opt:checked')).map((cb) => cb.value);
  return picked.length ? picked : [getCurrentBranch()];
}
...
function openTaskModal(mode, templateId) {
  ...
  if (mode === 'editTemplate') {
    ...
    const tplBranches = Array.isArray(tpl.branches) ? tpl.branches : [tpl.branch];
    renderTaskModalBranchGroup(tplBranches);
  } else {
    title.textContent = 'Add Task';
    submitBtn.textContent = 'Add Task';
    document.getElementById('taskForm').reset();
    document.getElementById('fieldRequired').checked = true;
    renderTaskModalBranchGroup([getCurrentBranch()]);
  }
...
function submitTaskForm(e) {
  ...
  const data = {
    ...
    branches: getSelectedBranchesFromModal(),
  };
...
/* ------------------------------------------------------------
   15b. TASK NOTE MODAL (optional note when completing/resetting a task —
   e.g. reporting a kendala/issue that caused a delay)
   ------------------------------------------------------------ */
let state_noteTaskId = null;
...
function openNoteModal(taskId) {
  ...
  state_noteTaskId = taskId;
  const willComplete = task.status !== 'Completed';
  document.getElementById('noteModalTitle').textContent = willComplete ? 'Complete Task' : 'Reset Task';
  document.getElementById('noteModalFieldLabel').textContent = willComplete
    ? 'Keterangan (opsional) — misal jika ada kendala'
    : 'Alasan reset (opsional)';
  document.getElementById('submitNoteBtn').textContent = willComplete ? 'Complete' : 'Reset';
  document.getElementById('fieldTaskNote').value = task.note || '';
  document.getElementById('noteModalOverlay').classList.add('open');
  setTimeout(() => document.getElementById('fieldTaskNote').focus(), 100);
}

function closeNoteModal() {
  document.getElementById('noteModalOverlay').classList.remove('open');
  state_noteTaskId = null;
}

function applyNoteModal(note) {
  if (!state_noteTaskId) return;
  const record = ensureTodayRecord();
  const task = record.tasks.find((t) => t.id === state_noteTaskId);
  if (!task) { closeNoteModal(); return; }
  toggleTaskStatus(state_noteTaskId, note);
  closeNoteModal();
}
...
function bindGlobalEvents() {
  ...
  // Dashboard summary cards → jump to Daily Checklist pre-filtered by status
  document.querySelectorAll('[data-goto-status]').forEach((card) => {
    card.addEventListener('click', () => {
      const status = card.dataset.gotoStatus;
      state.checklistFilter.status = status;
      state.checklistFilter.category = 'All';
      state.checklistFilter.search = '';
      navigate('checklist');
      // Sync the filter pill UI to match, since navigate() re-renders the
      // checklist page fresh each time.
  ...
  if (bulkDeleteBtn) {
    bulkDeleteBtn.addEventListener('click', () => {
      if (state.selectedTemplateIds.size === 0) return;
      bulkDeleteSelectedTemplates();
    });
  }
  ...
  // Task note modal (optional note on complete/reset)
  document.getElementById('noteForm').addEventListener('submit', (e) => {
    e.preventDefault();
    applyNoteModal(document.getElementById('fieldTaskNote').value);
  });
  document.getElementById('skipNoteBtn').addEventListener('click', () => applyNoteModal(''));
  ...
  document.getElementById('resetDataBtn').addEventListener('click', () => {
    openConfirmModal('This wipes every branch, every account, and every task for ALL devices — not just this one. This cannot be undone.', resetDemoData, 'Reset ALL clinic data?');
  });
  document.getElementById('logoutBtn').addEventListener('click', () => {
    openConfirmModal('You will need your username and password to log back in.', logout, 'Log out?');
  });

  // Access Control (Head of Clinic / Super Admin)
  ...
  document.getElementById('accessFieldRole').addEventListener('change', updateAccessBranchFieldVisibility);

  // Periodic history reset (Head of Clinic / Super Admin)
  const periodicResetBtn = document.getElementById('periodicResetBtn');
  if (periodicResetBtn) {
    periodicResetBtn.addEventListener('click', () => {
      const scope = document.getElementById('periodicResetScope').value;
      const branch = getCurrentBranch();
      const scopeLabel = scope === 'month' ? 'bulan berjalan' : 'seluruh riwayat';
      openConfirmModal(
        `Ini akan menghapus riwayat checklist (${scopeLabel}) untuk cabang ${branch}. Task template tidak terhapus. Tindakan ini tidak bisa dibatalkan.`,
        () => periodicResetHistory(scope),
        'Reset riwayat checklist?'
      );
    });
  }

  // Branch switcher (Super Admin only)
  document.getElementById('branchSwitcherSelect').addEventListener('change', (e) => {
    setViewingBranch(e.target.value);
    state.selectedTemplateIds.clear();
    ensureTodayRecord();
    state.historyFilter.date = null;
    renderAll();
  });
...
/* ------------------------------------------------------------
   17c. CLOUD SYNC BOOTSTRAP
   ------------------------------------------------------------
   On startup: pull whatever's already in Firestore for each data
   type (so a brand-new device inherits real clinic data instead of
   re-seeding demo data on top of it), THEN seed only what's still
   missing, THEN subscribe to live changes from other devices for
   the rest of the session. */
const CLOUD_KEYS_TO_SYNC = ['templates', 'records', 'staff', 'logs', 'settings'];

function pullCloudDataOnce() {
  if (!window.CloudSync || !window.CloudSync.isAvailable) return Promise.resolve();

  const pulls = CLOUD_KEYS_TO_SYNC.map((key) => new Promise((resolve) => {
    let settled = false;
    let unsubscribe = () => {};
    const finish = () => {
      if (settled) return;
    ...
    unsubscribe = window.CloudSync.subscribe(key, (value) => {
      localStorage.setItem(LS_KEYS[key], JSON.stringify(value));
      finish();
    });
    // If Firestore has nothing yet for this key (brand-new clinic), don't
    // hang forever waiting for a snapshot that will never satisfy our
    // "has data" check — give up after a short timeout and let seeding
    // fill it in locally (which will then push up and become the source
    // of truth for every other device).
...
/* Subscribe for the rest of the session: whenever another device changes
   data, patch our localStorage and re-render if we're logged in. Called
   once after the initial pull+seed above (re-subscribing is harmless
   since Firestore de-dupes listeners per doc, but we only need one). */
function subscribeToLiveChanges() {
  if (!window.CloudSync || !window.CloudSync.isAvailable) return;
  CLOUD_KEYS_TO_SYNC.forEach((key) => {
    window.CloudSync.subscribe(key, (value) => {
      localStorage.setItem(LS_KEYS[key], JSON.stringify(value));
      if (isLoggedIn()) {
        ensureTodayRecord();
        renderAll();
...
async function initApp() {
  ...
  // Wait briefly for the sync module to finish loading (it's a separate
  // <script type="module">, so it may init slightly after this file runs).
  await new Promise((resolve) => {
    if (window.CloudSync) return resolve();
    window.addEventListener('cloudsync-ready', resolve, { once: true });
    setTimeout(resolve, 3000); // don't block the app forever if it never loads
  });
